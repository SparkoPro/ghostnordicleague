...
bool CGamePlayer :: Update( void *fd )
{
	...
	if (m_Muted && m_WarnTime != 0 && GetTime() - m_WarnTime >= 30)
	{
		m_Muted = false;
		m_WarnTime = 0;
		m_Game->SendAllChat("Player [" + GetName() + "] has been unmuted after 30 sec auto mute due to spamming");
	}
			...
			case CGameProtocol :: W3GS_CHAT_TO_HOST:
				...
				if (m_MessageTimes[(1 + m_MTCursor)%MTNUM] != 0)
				{
					...
					if (dif < 3000) // 3 seconds
					{
						if (!m_Muted)
						{
							m_Game->SendAllChat("Player [" + GetName() + "] has been muted for 30 seconds due to spamming!");
							m_Game->SendChat(this, GetName() + ", if you continue to spam you will be kicked!");
							m_WarnTime = GetTime();
