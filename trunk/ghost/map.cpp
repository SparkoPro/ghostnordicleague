...
void CMap :: Load( CConfig *CFG, string nCFGFile )
{
	...
	m_HCLValidModes = CFG->GetString( "map_validhcl", string( ) );
	m_MapDownloadLink = CFG->GetString( "map_downloadlink", string( ) );
	m_MapSendDownloadLink =  CFG->GetInt( "map_senddownloadlink", 0 ) == 0 ? false : true;
