...
void CBaseGame :: EventPlayerJoined( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer )
{
	...
	if (AnyAdminCheck)
		m_AdminsInGame.push_back(Player->GetName());
...
else if( UTIL_IsLanIP( Player->GetExternalIP() ) )
{
	Player->SetSpoofed( true );
	...
	if( !m_GameLoading && !m_GameLoaded )
	{
		...
		*/
		...
		if (player->IsAdmin())
		{
			...
			{
				for( int i=0; i < m_AdminsInGame.size(); i++ )
				{
					if ( player->GetName() == m_AdminsInGame.at(i) )
						m_AdminsInGame.erase(m_AdminsInGame.begin()+i);
				}
			}
			else
		...
		if( chatPlayer->GetType( ) == CIncomingChatPlayer :: CTH_MESSAGE || chatPlayer->GetType( ) == CIncomingChatPlayer :: CTH_MESSAGEEXTRA )
		{
			...
			if( !Message.empty( ) && Message[0] == m_GHost->m_CommandTrigger )
			{
				...
				if( EventPlayerBotCommand( player, Command, Payload ) )
					Relay = false;
				if (m_MessageWasCommand && !m_GHost->m_RelayCommands)
					Relay = false;
			}
			
			if (!m_GHost->m_RelayCommands && Message[0] == '-' && Message.size() >= 3 && Message[1] != '.')
				Relay = false;
		...
		*/
		...
		else
		{
			...
			*/
			...
			{
				CONSOLE_Print( "[GAME: " + m_GameName + "] Sending download link [" + m_Map->GetMapDownloadLink() + "] to [" + player->GetName( ) + "]" );
				SendChat( player, m_Map->GetMapDownloadLink() );
			}
...
void CBaseGame :: EventGameLoaded( )
{
	...
	*/
	...
	if( m_AdminsInGame.size() > 0 )
	{
		CONSOLE_Print( "[ADMIN: " + m_GameName + "] One or more [ADMINS] are present in the game!");

		if ( m_GHost->m_DisplayAdminInGameMessage )
		{
			CONSOLE_Print( "[ADMIN: " + m_GameName + "] Displaying admin in game message from file [" + m_GHost->m_DisplayAdminInGameFile.c_str( ) + "]");

			Count = 0;
			Line = "";

			in.open( m_GHost->m_DisplayAdminInGameFile.c_str( ) );
