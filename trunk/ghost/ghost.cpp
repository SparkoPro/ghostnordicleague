...
void CONSOLE_RequestPrint(string message,string user)
{
	...
	strftime (buffer, 256, "%a, %b %d %Y", loctime);
	
	//Log File
	string File = "request.log";
			
	if( !File.empty ( ) )
	{		
		ofstream gLog;
		gLog.open(File.c_str( ), ios :: app );
		
		if( !gLog.fail( ))
		{
			//Display time
			gLog << "[" << user << ": " << buffer << "] " << message << endl;
			gLog.close( );
		}
	}
}
